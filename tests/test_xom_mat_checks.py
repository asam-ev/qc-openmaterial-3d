...
CHECKER_ID = xom_mat_checker.tables_sorted_correctly.CHECKER_ID
BASE_PATH = "tests/data/tables_sorted_correctly/"


def _load_result(monkeypatch, file_name):
    test_utils.create_test_config(os.path.join(BASE_PATH, file_name))
...
def test_tables_sorted_correctly_emp_positive(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_positive_emp.xompt")

    assert result.get_checker_status(CHECKER_ID) == StatusType.COMPLETED
    assert len(result.get_issues_by_rule_uid(RULE_UID)) == 0

    test_utils.cleanup_files()


def test_tables_sorted_correctly_emp_negative(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_negative_emp.xompt")
...
def test_tables_sorted_correctly_optical_positive(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_positive_optical.xompt")
...
def test_tables_sorted_correctly_optical_negative(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_negative_optical.xompt")
...
def test_tables_sorted_correctly_brdf_positive(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_positive_brdf.xompt")
...
def test_tables_sorted_correctly_brdf_negative(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_negative_brdf.xompt")
...
def test_tables_sorted_correctly_reflCoeff_positive(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_positive_reflCoeff.xompt")
...
def test_tables_sorted_correctly_reflCoeff_negative(monkeypatch) -> None:
    result = _load_result(monkeypatch, "tables_sorted_correctly_negative_reflCoeff.xompt")
